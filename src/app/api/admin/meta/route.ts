...
/** Built-in provider catalogue — kept in sync with GET /api/admin/providers/presets. */
...
/** GET /api/admin/meta — gateway configuration descriptor. */
export async function GET(req: NextRequest) {
  const origin = new URL(req.url).origin;

  const cfgRows = await db.systemConfig.findMany({
    where: { key: { in: ['admin_token', 'spoof_model', 'auto_fallback', 'hedge_delay', 'cache_ttl'] } },
  });
  const cfg = (k: string): string | null => cfgRows.find((c) => c.key === k)?.value ?? null;
  ...
  return NextResponse.json({
    ...
    admin_token: cfg('admin_token') ?? 'nova-admin-token',
  });
}
